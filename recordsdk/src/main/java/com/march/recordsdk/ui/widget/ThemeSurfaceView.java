package com.march.recordsdk.ui.widget;
...
public class ThemeSurfaceView extends SurfaceView implements Callback {
	...
	public void setOnComplateListener(OnCompleteListener l) {
	...
	public void setIntent(Intent intent) {
		if (intent != null) {
			mImportImage = intent.getBooleanExtra(RecordConstant.EXTRA_MEDIA_IMPORT_IMAGE, false);
			boolean mImportVideo = intent.getBooleanExtra(RecordConstant.EXTRA_MEDIA_IMPORT_VIDEO, false);
...
//			}
	...
	/** MV滤镜公共路径 */
	public void setFilterCommonPath(String path) {
	...
	public void setVideoEndPath(String path) {
		this.mEndPath = path;
		this.mEndWidth = RecordCommonHelper.getInt(RecordConstant.THEME_LOGO_AUTHOR_WIDTH, 0);
		this.mEndHeight = RecordCommonHelper.getInt(RecordConstant.THEME_LOGO_AUTHOR_HEIGHT, 0);
	}
...
//	 * @param outPcm 合并渲染输出一个pcm文件
	...
	public void initFilter() {
		if (mSurfaceCreated && mSurfaceHolder != null) {
			...
			UtilityAdapter.FilterParserFree();
			//预览
			Log.i("chendong","before ");
			String setting = getSetting(false);
			UtilityAdapter.FilterParserInit(setting, mSurfaceHolder.getSurface());
			Log.i("chendong","after  " + setting);
		} else {
			mNeedInit = true;
		}
	}
	...
	private String getSetting(boolean output) {
		...
		if (mMediaObject != null) {
			...
			if (mMediaObject.getMediaParts() != null) {
				final int count = mMediaObject.getMediaParts().size();
				if (count > 1) {
					int duration = mMediaObject.getMediaParts().get(0).duration;
			...
			settings.append(RecordSDK.getConfig().getVideoBitrate());//mMediaObject.mVideoBitrate
			settings.append("; ");
		}
		//随机因子
