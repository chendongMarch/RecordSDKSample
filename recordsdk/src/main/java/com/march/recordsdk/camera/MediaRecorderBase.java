...
public abstract class MediaRecorderBase implements Callback, PreviewCallback, IMediaRecorder {
	...
	/** 转码Handler */
	protected EncodeHandler mEncodeHandler;
	/** 拍摄存储对象 */
	...
	public void setOutputSize(int height,int width) {
		this.height = height;
		this.width = width;
	}
	...
	public void setOnEncodeListener(OnEncodeListener l) {
		...
		mEncodeHandler = new EncodeHandler(this);
	}
	...
	public void startEncoding() {
		if (mMediaObject == null || mEncodeHandler == null)
			return;

		mEncodeHandler.removeMessages(MESSAGE_ENCODE_PROGRESS);
		mEncodeHandler.removeMessages(MESSAGE_ENCODE_COMPLETE);
		mEncodeHandler.removeMessages(MESSAGE_ENCODE_START);
		mEncodeHandler.removeMessages(MESSAGE_ENCODE_ERROR);
		mEncodeHandler.sendEmptyMessage(MESSAGE_ENCODE_START);
	}
	...
	protected void concatVideoParts() {
		new AsyncTask<Void, Void, Boolean>() {
			...
			protected void onPostExecute(Boolean result) {
				if (result) {
					mEncodeHandler.sendEmptyMessage(MESSAGE_ENCODE_COMPLETE);
				} else {
					mEncodeHandler.sendEmptyMessage(MESSAGE_ENCODE_ERROR);
				}
			}
		}.execute();
	}
